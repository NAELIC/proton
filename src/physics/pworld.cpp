...
void PWorld::handleCollisions(dGeomID o1, dGeomID o2) {
    PSurface* sur;
    int j =
        sur_matrix[*((int*)(dGeomGetData(o1)))][*((int*)(dGeomGetData(o2)))];
    if (j != -1) {
        const int N = 10;
        dContact contact[N];
...
void PWorld::initAllObjects() {
    ...
    if (flag) {
        for (int i = 0; i < surfaces.size(); i++)
            sur_matrix[(*(int*)(dGeomGetData(surfaces[i]->id1)))][*(
                (int*)(dGeomGetData(surfaces[i]->id2)))] =
...
PSurface* PWorld::createSurface(PObject* o1, PObject* o2) {
    ...
    sur_matrix[o1->id][o2->id] = sur_matrix[o2->id][o1->id] =
        surfaces.size() - 1;
    return s;
}
